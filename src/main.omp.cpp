...
#define OMP_THREAD_COUNT thread::hardware_concurrency()
#endif

#ifndef OMP_SCHEDULE_KIND
#define OMP_SCHEDULE_KIND static
#endif
...
vector<genome> epoch(unsigned int pop_size, vector<genome> &genomes)
{
    ...
    #pragma omp parallel for num_threads(OMP_THREAD_COUNT) schedule(OMP_SCHEDULE_KIND)
