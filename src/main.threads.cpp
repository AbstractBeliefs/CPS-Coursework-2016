...
#ifndef THREAD_COUNT
#define THREAD_COUNT thread::hardware_concurrency()
#endif
...
vector<genome> epoch(unsigned int pop_size, vector<genome> &genomes)
{
    ...
    auto make_babies = [&pop_size, &fitness, &genomes, &babies](int begin, int end){
        for (unsigned int i = begin; i < end; i += 2)
        {
    ...
    size_t start = NUM_ELITE * NUM_COPIES_ELITE;
    size_t end = pop_size;
    size_t chunk_size = (end - start) / THREAD_COUNT;   // Watch for truncation here if you have
                                                        // strange pop/elite values vs thread_count
    for (size_t thread_number = 0; thread_number < THREAD_COUNT; thread_number++){
        threads.push_back(thread(make_babies, thread_number*chunk_size, thread_number*chunk_size + chunk_size));
    }
